...
`worklog/` のファイルには、末尾に次の欄を必ず置く。ハーネスを直す唯一の材料になる。
...
- 効かなかった指示:
- hook が止めた回数と内容:
- advisor / plan-reviewer の指摘の採否:
```

書くことが無ければ各行に「なし」と書く。欄ごと省かない。
「なし」が並んでいること自体が、ルールが効いている証拠として読める。
事故が起きたセッションでは必ず埋める。

この欄は `skill: harness-review` が横断して読む。
